...
# bullet speed in ratio of screen height per sec in relation to the firing ship
BULLET_VEL_R = 0.6
...
def collisionvels(obj1, obj2, App):
    ...
    u1 = ((2 * m2 / (m1 + m2)) *
          (np.dot(v1 - v2, x1 - x2) / np.dot(x1 - x2, x1 - x2)))
    u2 = ((2 * m1 / (m1 + m2)) *
          (np.dot(v2 - v1, x2 - x1) / np.dot(x2 - x1, x2 - x1)))
...
class App():
    ...
    def __init__(self, w=0, h=0, frames=60):
        ...
        self.screen = None
        if w == 0 and h == 0:
            self.size = self.width, self.height = pygame.display.Info(
    ...
    def on_init(self):
        ...
        self.normal_font = pygame.font.Font(
            None, int(NORMAL_FONT_R * self.height))
        self.small_font = pygame.font.Font(
            None, int(SMALL_FONT_R * self.height))
        ...
        if self.bg_rect.width > self.width:
            self.bg = pygame.transform.scale(
                self.bg, (self.width, self.width / self.bg_ar))
        elif self.bg_rect.width > self.width:
            self.bg = pygame.transform.scale(
                self.bg, (self.height * self.bg_ar, self.height))
        else:
            self.tile = True

        for n in range(EXPLOSION_FRAMES):
            self.explosionimg.append(pygame.image.load(
                "images/explosion/" + str(n + 1) + '.png').convert_alpha())

        for n in range(FLAME_FRAMES):
            self.flameimg.append(pygame.image.load(
                "images/flame" + str(n + 1) + '.png').convert_alpha())

        for n in range(SMOKE_FRAMES):
            self.smokeimg.append(pygame.image.load(
                "images/smoke" + str(n + 1) + '.png').convert_alpha())
        ...
        for n in range(MAX_PLAYERS):
            self.playerimg.append(pygame.image.load(
                "images/player" + str(n + 1) + '.png').convert_alpha())
            self.playerimg_shield.append(pygame.image.load(
                "images/player" + str(n + 1) + '_shield2.png').convert_alpha())
        ...
        self.text_time = self.small_font.render(
            TIME_S1 + "{:.1f}".format(time.time() - self.t0) + TIME_S2, True, WHITE)
        self.text_fps = self.small_font.render(
            str(int(self.clock.get_fps())) + FPS_S, True, WHITE)

        self.bulletimg = pygame.image.load(
            'images/bullets.png').convert_alpha()
    ...
    def PlayerHitPlayer(self):
        ...
        for n in range(MAX_PLAYERS):
            for p in range(MAX_PLAYERS):
                if p != n:
                    hits.append(pygame.sprite.groupcollide(self.players[n], self.players[
                                p], False, False, collided=pygame.sprite.collide_mask))
        for hit in hits:
    ...
    def BulletHitPlayer(self):
        ...
        for n in range(MAX_PLAYERS):
            for p in range(MAX_PLAYERS):
                if p != n:
                    hits.append(pygame.sprite.groupcollide(self.players[n], self.players[
                                p].sprites()[0].bullets, False, False, collided=pygame.sprite.collide_mask))
        for hit in hits:
                    ...
                    if bullet not in agent.bullets:
                        if agent.last_collision != bullet:
                            ...
                            if agent.collide:
                                agent.hp -= 1
                                agent.collisiontime = time.time()
                                Explosion(self, bullet,
                                          self.explosions, self.all_sprites)
                                bullet.kill()
                            else:
                                bullet.kill
                                bullet.add(agent.bullets,
        ...
        if self.tile:
            for n in range(int(m.ceil(self.width / self.bg_rect.width))):
                for p in range(int(m.ceil(self.height / self.bg_rect.height))):
                    self.screen.blit(
                        self.bg, (n * self.bg_rect.width, p * self.bg_rect.height))
        else:
            self.screen.blit(self.bg, (0, 0))

        self.screen.blit(self.text_fps, ((1 - BORDER_W_R) * self.width - self.text_fps.get_width(),
                                         (1 - BORDER_BOTTOM_R) * self.height - self.text_fps.get_height()))
        self.screen.blit(self.text_time, (BORDER_W_R * self.width,
                                          (1 - BORDER_BOTTOM_R) * self.height - self.text_time.get_height()))

        for agent in self.agents:
            self.screen.blit(agent.text_player, ((1 - agent.number) * BORDER_W_R * self.width + agent.number * ((1 - BORDER_W_R)
                                                                                                                * self.width - agent.text_player.get_width() - agent.playerlogo.get_width()), BORDER_H_R * self.height))
            self.screen.blit(agent.text_hp, ((1 - agent.number) * BORDER_W_R * self.width + agent.number * ((1 - BORDER_W_R)
                                                                                                            * self.width - agent.text_hp.get_width()), BORDER_H_R * self.height + agent.text_player.get_height()))
            self.screen.blit(agent.text_score, ((1 - agent.number) * BORDER_W_R * self.width + agent.number * ((1 - BORDER_W_R) * self.width -
                                                                                                               agent.text_score.get_width()), BORDER_H_R * self.height + agent.text_player.get_height() + agent.text_hp.get_height()))
            self.screen.blit(agent.text_speed, ((1 - agent.number) * BORDER_W_R * self.width + agent.number * ((1 - BORDER_W_R) * self.width -
                                                                                                               agent.text_speed.get_width()), BORDER_H_R * self.height + agent.text_player.get_height() + agent.text_hp.get_height() + agent.text_score.get_height()))
            self.screen.blit(agent.playerlogo, ((1 - agent.number) * (BORDER_W_R * self.width + agent.text_player.get_width(
            )) + agent.number * ((1 - BORDER_W_R) * self.width - agent.playerlogo.get_width()), BORDER_H_R * self.height))
...
class Human(pygame.sprite.Sprite):
    ...
    def on_init(self, App):
        self.dead = False
        self.score += 1
        self.pos = np.array([np.random.random_sample() * App.width * (1 - 2 * GAME_W_R) + GAME_W_R * App.width,
                             np.random.random_sample() * App.height * (1 - 2 * GAME_H_R) + GAME_H_R * App.height]).reshape((2, 1))
        self.wrap(App)
        ...
        self.text_speed = App.small_font.render(
            SPEED_S1 + "{:3.1f}".format(norm(self.vel) * 100 / (self.MAX_SPEED)) + SPEED_S2, True, WHITE)
        self.text_score = App.normal_font.render(
            SCORE_S + str(self.score), True, WHITE)
        self.text_hp = App.normal_font.render(HP_S + str(self.hp), True, WHITE)
        self.playerlogo = pygame.transform.scale(App.playerimg[self.number], (int(App.playerimg[self.number].get_width(
        ) * self.text_player.get_height() / App.playerimg[self.number].get_height()), int(self.text_player.get_height())))
        ...
        self.aspect_ratio = float(
            self.original.get_width()) / float(self.original.get_height())
    ...
    def resize(self, App):
        self.original = pygame.transform.smoothscale(self.original, (int(
            self.aspect_ratio * self.scale * App.height), int(self.scale * App.height)))
        self.original_shield = pygame.transform.smoothscale(self.original_shield, (int(
            SHIELD_IMG_R * self.aspect_ratio * self.scale * App.height), int(SHIELD_IMG_R * self.scale * App.height)))
    ...
    def update(self, App):
        ...
        if self.emitter.visible or self.emitter.smoke:
            self.emitter.pos = self.pos - \
                VectorfromAngle(self.angle) * self.mask_orig.get_size()[0]
...
class Emitter(pygame.sprite.Sprite):
    ...
    def __init__(self, Human, App, *sprite_groups):
        ...
        self.frame = 0
        self.angle = Human.angle
        # -VectorfromAngle(self.angle)*App.agents.sprites()[self.number].original.get_width()
        self.pos = Human.pos
        ...
        for n in range(len(App.flameimg)):
            self.aspect_ratio.append(
                float(self.original[n].get_width()) / float(self.original[n].get_height()))

        for n in range(len(App.smokeimg)):
            self.aspect_ratio_smoke.append(float(self.original_smoke[
                                           n].get_width()) / float(self.original_smoke[n].get_height()))
    ...
    def resize(self, App):
        for n in range(len(self.original)):
            self.original[n] = pygame.transform.smoothscale(self.original[n], (int(
                self.aspect_ratio[n] * self.scale * App.height), int(self.scale * App.height)))
        for n in range(len(self.original_smoke)):
            self.original_smoke[n] = pygame.transform.smoothscale(self.original_smoke[n], (int(
                self.aspect_ratio_smoke[n] * self.scale_smoke * App.height), int(self.scale_smoke * App.height)))
    ...
    def rotate(self, angle):
        if self.smoke:
            self.image = pygame.transform.rotate(
                self.original_smoke[self.frame % FLAME_FRAMES], m.degrees(angle))
        elif self.visible:
            self.image = pygame.transform.rotate(
                self.original[self.frame % FLAME_FRAMES], m.degrees(angle))
        else:
            self.image = pygame.transform.rotate(
                self.original[self.frame % FLAME_FRAMES], m.degrees(angle))
    ...
    def load(self, App):
        ...
        if self.smoke:
            self.rect.move_ip(tuple(self.pos - tonumpy(self.rect.center) +
                                    VectorfromAngle(self.angle) * self.rect.width / 3))
        else:
            self.move()
